 ...
 * 
 */
public class ChildrensClothing extends Product{
	...
	public int sellingPrice(int[] date){
		...
		if (differenceDays <= 365){
			return Math.round((float)((cost*1.50)*1.05));//cast float to keep precision 
		} else if (differenceDays < 0){
			return -1;	//you can't buy a shirt before it was bought by the store
		}else {
			return Math.round((float)((cost*1.10)*1.05));//cast float to keep precision 
		}
	}
}
